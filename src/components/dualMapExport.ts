...
function applyLayerTypeBadge(context: CanvasRenderingContext2D, width: number, layerType: string) {
  ...
  context.fillRect(10, 10, 220, 34);
  context.strokeStyle = 'rgba(255, 255, 255, 0.28)';
  context.strokeRect(10, 10, 220, 34);
...
export async function downloadSatellitePack(options: DownloadSatellitePackOptions): Promise<void> {
  ...
  const visNightFactor = autoReduceVisAtNight && activeLayers.vis && activeLayers.ir
    ...
    : 1;
  const exportSandwichOpacity = sandwichOpacity * visNightFactor;
  const exportHybridVisOpacity = rgbHdOpacity * visNightFactor;
  ...
  const needsIr = selectedKinds.has('ir') || selectedKinds.has('sandwich') || selectedKinds.has('hybrid');
  ...
  const composeVisOverlayCanvas = (
    ...
    blendMode: GlobalCompositeOperation = 'luminosity',
  ) => {
  ...
  const rgbHdCanvas = shouldExportHd ? composeVisOverlayCanvas(rgbTempCanvas, rgbHdOpacity) : null;
  const sandwichCanvas = shouldExportSandwich ? composeVisOverlayCanvas(irTempCanvas, exportSandwichOpacity) : null;
  const hybridCanvas = shouldExportHybrid ? (() => {
    ...
    const visData = visTempCtx.getImageData(0, 0, width, height).data;
    const irData = irTempCtx.getImageData(0, 0, width, height).data;

    for (let i = 0; i < cloudOnlyIrData.length; i += 4) {
      const visLum = (visData[i] + visData[i + 1] + visData[i + 2]) / 3;
      const cloudMask = Math.min(1, Math.max(0, (visLum - 90) / 100));
      ...
      cloudOnlyIrData[i + 3] = Math.round(255 * cloudMask * exportHybridIrOpacity);
    }
